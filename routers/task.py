...
async def list_tasks():
    ...
    # 실체 DB가 없으므로, 예시 데이터를 직접 만들어 응답으로 보낸다.
    # task_schema.Task(...) 형태로 모델을 맞춰 값을 채운다.
...
async def update_task(task_id: int, task_body: task_schema.TaskCreate):
    ...
    # pydantic v2 기준: task_body.dict() = task-body_dump() 로 변경
